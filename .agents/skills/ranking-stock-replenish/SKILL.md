...
**確認待ちリストの照合**(Phase 19-5 で導入): 在庫確認の前に `docs/plans/pref-ranking-1.html` の「確認待ちリスト」を読み、トリガー列が**「次の週次補充」**の行があれば今回の補充で確認し、結果列に日付 + 結果を書く(OK なら行を `pref-ranking-1-log.html` へ移す。NG なら「NG 時の扱い」列に従う)。該当なしなら「確認待ち: 該当なし」と一言宣言して進む。リストの節が未設置なら該当なしとして進む。ルールは `docs/plans/index.html` セクション 5.2。

**課題表の照合(セット名)**(Phase 22-4 で導入。ルールは `docs/issues/index.html` セクション 4.8): 同じく在庫確認の前に `grep -n "pref-ranking-1" docs/issues/index.html` で課題体系の live 表(pref-ranking-1 の節 + 開発レーンの節でセット名を含む行)を引き、トリガー列が**「pref-ranking-1 の次の週次補充」「第 N バッチ整備時」**(セット型)の行を拾う。該当行があれば、今回の補充で「同梱する(該当するゴールの完了条件に足す)/ トリガーを更新して据え置き継続 / 受容」を決めて表を更新する(黙って通り過ぎない)。該当なしなら「課題表: 該当なし」と一言宣言して進む。
...
- **採否ルール(17-3 確立)**:
  ...
  - **時刻・時分の指標(平均就寝時刻・「1時間45分」)は現行データモデルに載らない**ため見送り(課題体系 `docs/issues/index.html` の `I-023`。分換算の採否は未決)
...
5. **締め**: 在庫確認クエリで両環境 `unbuilt_30s = 0` を確認 → バッチ資材・台帳の変更をコミット + push → セット記録 `docs/plans/pref-ranking-1-log.html` の「週次補充の記録」へ表 1 行（+ 特記があれば本文）を追記し、セット計画書 `docs/plans/pref-ranking-1.html` のステータス欄の在庫情報も更新する（Phase 18 の 2 レーン化で記録先を変更）
   - **計画書の様式**(25-5〔2026-09-22〕で更新): `docs/plans/pref-ranking-1.html` は **Phase 25 の新様式へ移行済み**で構造検査の対象。改修 R-x を `section.plan-phase`、ステップ R-x-n を `article.plan-step` で書き(枠は `docs/plans/_templates/step.html`、様式の正は `docs/plans/index.html` セクション 5.0)、**ステータス欄・確認待ちリストは表のまま**更新する。**記録ファイル `pref-ranking-1-log.html` は未移行**(従来どおり)。計画書を書き換えたら**コミット前に `python3 tools/docs_check.py --plan-only` を実行し出力を会話に貼る**(確認待ちリストの結果列だけを直した場合も同じ)。**各セットの補充工程・在庫検査・人間ゲートは変更しない**
