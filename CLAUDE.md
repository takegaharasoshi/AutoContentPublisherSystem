...
- **Phase D〜15・17（設計 → インフラ構築 → アプリ実装 → 動画対応 → 勝負セット → 第 3 セット pref-ranking-1）は完了済み**。現在の開発レーンは **Phase 16 の残り（16-5 効果検証 → 16-12 指標セット確定）** と **Phase 19（開発プロセスへの `/goal` 組み込み。19-1〜19-4 完了、残りは 19-5〔入口スキルへの「確認待ちの参照」組み込み〕。各スキルの初回試行は計画書の「確認待ちリスト」で追う）**。プラットフォーム展開は Phase XX として着手判断待ち（現況は計画書を参照）
