...
- **Phase D〜15・17（設計 → インフラ構築 → アプリ実装 → 動画対応 → 勝負セット → 第 3 セット pref-ranking-1）は完了済み**。**16-5・16-12a は 2026-09-02 に完了**（現行フォーマットの水準評価・指標セット確定。集計ランナーは `content/insights-analysis/`）。現在の開発レーンは **16-12b（指標セット確定の反映）と Phase 20（fantasy-animals-1 の停止・2026-09-02 決定）**。**Phase 19（開発プロセスへの `/goal` 組み込み）は 2026-09-02 に完了**（`/step`・補充スキル 2 本・`/idea` に確認待ちリストの照合と起票ルールを組み込み済み。各スキルの初回試行は計画書末尾の「確認待ちリスト」でトリガー付きに追う）。プラットフォーム展開は Phase XX として着手判断待ち（現況は計画書を参照）
