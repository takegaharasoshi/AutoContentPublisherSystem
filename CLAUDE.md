...
- **移行済み計画書を更新したら構造検査を行う**（Phase 25 で導入。2026-09-21 時点の移行済みは `docs/plans/development-plan.html` のみ）: 新様式の雛形は `docs/plans/_templates/step.html` をコピーし、更新後に `python3 tools/docs_check.py --plan-only`（構造 + 参照のみ・ブラウザー不要）を実行する。CSS・テンプレートを変えたとき、または新しい計画書を移行したときは `python3 tools/docs_check.py --widths 360,390,768,1280` も回す。**セット計画書は未移行**のため従来様式のまま書く（`docs/plans/index.html` セクション 5.0）
- **完了済みフェーズ**: Phase D〜20・22〜24（設計 → インフラ構築 → アプリ実装 → 動画対応 → 勝負セット → インサイト収集と KPI 運用 → 第 3 セット pref-ranking-1 → 2 レーン化 → `/goal` 組み込み → fantasy-animals-1 の停止 → 課題体系の新設 → Codex 直接セッションの導入 → 設計書 HTML のスマホ閲覧対応〔2026-09-21。一般表の列幅対策・課題 / アイデア一覧のカード化・検証ツール `tools/docs_check.py`〕）。要点は `docs/plans/development-plan.html` の「完了フェーズのサマリ」、経緯の全文は `development-log.html`。稼働中のセットは logic-training-1・pref-ranking-1 の 2 本。月次 KPI 転記の手順は `docs/app/operation.html` セクション 6.3
- **進行中の開発レーンは Phase 21**（ウミガメのスープ参加型セット・第 4 セット。キャラ名「探偵カメロック」。設計書は `docs/app/sets/umigame-soup-1.html`・`docs/app/generators/umigame-prebuilt.html`、PoC 資材は `poc/umigame-comment-webhook/`、動画資材は `content/video-build/umigame-soup-1/`、作問スキルは `.agents/skills/umigame-problem-writer/`）**と Phase 25**（計画書の可読性改善と記載様式の定着）**の 2 本**。ステップ単位の進捗・次の着手は `docs/plans/development-plan.html` を見る（ここには重複して書かない）
