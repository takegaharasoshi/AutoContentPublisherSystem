# 2026-09-w1 補充: stock_items.py から review.md(人間レビュー用)と insert_quiz_stock.sql(投入用)を生成
import json
import sys
...
# ---------- review.md ----------
lines = [
    "# 2026-09-w1 問題ストック補充: 14 問レビューシート",
    "",
    ...
    "## 着手時の在庫(2026-09-03)",
    "",
    "- 朝(L1/light)= 未使用かつビルド済み 6 問 / 夜(L1/deep)= 5 問。どちらも閾値 7 問未満(unbuilt は 0)。週の消費が各 7 問のため各 7 問を補充",
    ...
    "3. **既存ストック・出題履歴との重複**: 既存ストック 77 問(2026-08-initial 42 + 2026-08-w3 21 + 2026-08-w5 14)と Aurora の quiz_items 92 件の summary と突合済み(重複なし)。投入直前に Aurora の quiz_items 全件と再突合する",
