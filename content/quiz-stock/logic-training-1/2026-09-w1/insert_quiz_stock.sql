-- 2026-09-w1 問題ストック補充投入(14 問。レビュー承認後に実行)
-- 生成元: content/quiz-stock/logic-training-1/2026-09-w1/stock_items.py(単一ソース)。適用先: ローカル MySQL / Aurora(acps)
...
VALUES ((SELECT id FROM batch_sets WHERE set_code = 'logic-training-1'),
        ...
        'L1', 'light',
        '切れば切るほど、公平になるもの。さて、なんだ?',
        'トランプ(切る=シャッフルする)',
        '{"hook":"切るほど良くなるものがある?","hint":"包丁じゃない切り方だ!","question":"切れば切るほど、公平になるもの。さて、なんだ?","answer":"トランプ(切る=シャッフルする)","explanation":"トランプは「切る」と言うが、刃物で切るのではなく混ぜること。よく切るほど順番の偏りが消えて、誰が引いても公平になる。「切る」の別の意味に気づけるかが勝負だ。","coach_comment":"言葉の裏の意味まで読めたな!","tags":["なぞなぞ","朝の一問","言い回し"],"summary":"「切っても切れないもの=トランプ」の定番を、「切るほど公平になるもの」と問い直した言い回しなぞなぞ。切る=シャッフルの二義性に気づかせる。別解(水・切手・縁)は「公平」の条件で封じた。","illustration_scene":"朝日が差し込むキッチンのまな板の上に、切れ味のよさそうな包丁が置かれ、その横に大きな「?」が浮かんでいる。トランプ・カードは描かない。文字・数字は描かない。人物は描かない。"}',
        '類型: 言い回し(切る=シャッフル)の定番なぞなぞ(作者不詳)。流布例: https://detail.chiebukuro.yahoo.co.jp/qa/question_detail/q13120224083 , https://peing.net/ja/q/210dfad0-9ee9-43ee-a9b2-da00267af7a9 , https://comotto.docomo.ne.jp/column/00000283-2/ 。問い方を「公平になる」へ変えて出題。文面はオリジナルに書き下ろし(表現は書き直し済み)。', 1);
...
VALUES ((SELECT id FROM batch_sets WHERE set_code = 'logic-training-1'),
        (SELECT CONCAT('morning-', LPAD(COALESCE(MAX(CAST(SUBSTRING_INDEX(t.content_key, '-', -1) AS UNSIGNED)), 0) + 1, 3, '0')) FROM (SELECT q.content_key FROM quiz_stock_items q JOIN batch_sets b ON b.id = q.set_id WHERE b.set_code = 'logic-training-1' AND q.content_key LIKE 'morning-%') t),
        'L1', 'light',
        '1、11、21、1211、111221。ある決まりで並んでいる。次に来る数はなんだ?',
        '312211(1が3個、2が2個、1が1個)',
        '{"hook":"数学者も遊んだ有名な数列だ","hint":"計算するな、声に出して読め!","question":"1、11、21、1211、111221。ある決まりで並んでいる。次に来る数はなんだ?","answer":"312211(1が3個、2が2個、1が1個)","explanation":"前の数を「何が何個」と読むと次の数になる。1→1が1個→11。11→1が2個→21。21→2が1個、1が1個→1211。1211→1が1個、2が1個、1が2個→111221。111221→1が3個、2が2個、1が1個→312211。","coach_comment":"読み方ひとつで世界が変わる!","tags":["なぞなぞ","朝の一問","法則発見"],"summary":"1、11、21、1211、111221の次を問う「見て言う数列」(look-and-say)。前の数を「何が何個」と読み上げると次の数になる法則を見抜く。答えは312211で一意。","illustration_scene":"朝日が差し込む教室の黒板に、チョークで「1」「11」「21」「1211」「111221」「?」の6行が縦に大きく書かれている。窓の外は青空と朝日。文字はこの6行だけを例外的に描き、他の文字・数字は描かない。人物は描かない。"}',
        '類型: 法則発見(見て言う数列・国際的に流布する古典パズル)。流布例: https://detail.chiebukuro.yahoo.co.jp/qa/question_detail/q1122764576 , https://japontimes.livedoor.biz/archives/54524968.html , https://www.quora.com/What-is-the-next-number-in-this-sequence-1-11-21-1211-111221-312211-13112221-2 。文面はオリジナルに書き下ろし(表現は書き直し済み)。', 1);
...
VALUES ((SELECT id FROM batch_sets WHERE set_code = 'logic-training-1'),
        (SELECT CONCAT('morning-', LPAD(COALESCE(MAX(CAST(SUBSTRING_INDEX(t.content_key, '-', -1) AS UNSIGNED)), 0) + 1, 3, '0')) FROM (SELECT q.content_key FROM quiz_stock_items q JOIN batch_sets b ON b.id = q.set_id WHERE b.set_code = 'logic-training-1' AND q.content_key LIKE 'morning-%') t),
        'L1', 'light',
        '漢字の「日」に一画だけ足すと、別の漢字ができる。さて、いくつ作れる? 目標は8つだ。',
        '目・田・白・旧・旦・甲・由・申(8字)',
...
VALUES ((SELECT id FROM batch_sets WHERE set_code = 'logic-training-1'),
        ...
        'L1', 'deep',
        '目盛りのない3Lと5Lの容器がある。水は好きなだけ使えて、捨ててもいい。ぴったり4Lを量るには、どう注げばいい?',
        '5Lを満たし3Lへ移す(残り2L)→3Lを空けて2Lを入れる→5Lを満たし、3Lが満杯になるまで注ぐ→5Lに4L残る',
        '{"hook":"頭の体操の王様、いけるか?","hint":"あふれさせて差を作れ!","question":"目盛りのない3Lと5Lの容器がある。水は好きなだけ使えて、捨ててもいい。ぴったり4Lを量るには、どう注げばいい?","answer":"5Lを満たし3Lへ移す(残り2L)→3Lを空けて2Lを入れる→5Lを満たし、3Lが満杯になるまで注ぐ→5Lに4L残る","explanation":"5Lを満たして3Lへ移すと、5L側に2L残る。3Lを空けて2Lを移す。もう一度5Lを満たし、3L側が満杯になるまで注ぐと移るのは1Lだけ。5L側に4Lが残る。","coach_comment":"手を動かせば道が見えるぞ!","tags":["水平思考","夜の一問","容量パズル"],"summary":"目盛りのない3Lと5Lの容器でちょうど4Lを量る古典パズル。5Lから3Lへ移して2Lを作り、それを足場に最後の1Lだけを移す手順を問う。","illustration_scene":"夜の台所。照明の下に大きさの違う透明な容器が2つ並び、蛇口から水が注がれている。容器に目盛りはない。宙に大きな「?」。数字・文字は描かない。人物は描かない。"}',
        '類型: 容量パズル(国際的古典・作者不詳)。流布例: https://detail.chiebukuro.yahoo.co.jp/qa/question_detail/q11103013526 , https://sairyu-sensei.com/3l5l4l/ , https://www.plies.co.jp/single-post/2017/08/28/4%E3%83%AA%E3%83%83%E3%83%88%E3%83%AB%E3%81%AE%E6%B0%B4%EF%BC%88%E7%AD%94%E3%81%88%EF%BC%89 。文面はオリジナルに書き下ろし(表現は書き直し済み)。', 1);
...
VALUES ((SELECT id FROM batch_sets WHERE set_code = 'logic-training-1'),
        (SELECT CONCAT('night-', LPAD(COALESCE(MAX(CAST(SUBSTRING_INDEX(t.content_key, '-', -1) AS UNSIGNED)), 0) + 1, 3, '0')) FROM (SELECT q.content_key FROM quiz_stock_items q JOIN batch_sets b ON b.id = q.set_id WHERE b.set_code = 'logic-training-1' AND q.content_key LIKE 'night-%') t),
        'L1', 'deep',
        '机に硬貨が20枚。表向きはちょうど10枚だが、目隠しをしていて触っても表裏は分からない。表の枚数が同じ2つの山に分けるには?',
        '10枚を取り分け、その10枚を全部裏返す',
        '{"hook":"目隠しでも数は操れる","hint":"ひっくり返すのは自由だ!","question":"机に硬貨が20枚。表向きはちょうど10枚だが、目隠しをしていて触っても表裏は分からない。表の枚数が同じ2つの山に分けるには?","answer":"10枚を取り分け、その10枚を全部裏返す","explanation":"取った10枚に表がn枚なら、残りの表は10−n枚。取った10枚を全部裏返すと、表はちょうど10−n枚になり、残りと同じ数になる。nが何枚でも成り立つ。","coach_comment":"見えなくても理屈は裏切らない!","tags":["水平思考","夜の一問","論理パズル"],"summary":"表10枚を含む硬貨20枚を目隠しで表の枚数が等しい2山に分ける古典パズル。10枚を取り分けて全部裏返すと、取り分けた側の表の枚数が残りと一致する。","illustration_scene":"夜、デスクランプに照らされた木の机に、たくさんの硬貨が散らばっている。硬貨は無地で数字や模様は描かない。机の端に目隠し用の布が置かれ、宙に大きな「?」。文字は描かない。人物は描かない。"}',
        '類型: 不変量を使う手順パズル(作者不詳の定番)。流布例: https://sist8.com/10co , https://www.soranokillingtime.com/analytical-puzzle/heads-10-coins/ , https://diamond.jp/articles/-/340346 。文面はオリジナルに書き下ろし(表現は書き直し済み)。', 1);
...
VALUES ((SELECT id FROM batch_sets WHERE set_code = 'logic-training-1'),
        (SELECT CONCAT('night-', LPAD(COALESCE(MAX(CAST(SUBSTRING_INDEX(t.content_key, '-', -1) AS UNSIGNED)), 0) + 1, 3, '0')) FROM (SELECT q.content_key FROM quiz_stock_items q JOIN batch_sets b ON b.id = q.set_id WHERE b.set_code = 'logic-training-1' AND q.content_key LIKE 'night-%') t),
        'L1', 'deep',
        '深さ10mの井戸の底にカタツムリがいる。昼に3m登り、夜は眠って2mずり落ちる。井戸の上に出られるのは何日目?',
        '8日目(7日で7m、8日目の昼に到達)',
        '{"hook":"計算のようで計算じゃない","hint":"最終日はずり落ちない!","question":"深さ10mの井戸の底にカタツムリがいる。昼に3m登り、夜は眠って2mずり落ちる。井戸の上に出られるのは何日目?","answer":"8日目(7日で7m、8日目の昼に到達)","explanation":"1日で差し引き1mだから10日、は早合点。7日目の終わりに7m。8日目の昼に3m登った時点で10mに届いて上に出るので、その夜はもう落ちない。","coach_comment":"最後の一歩を見落とすな!","tags":["水平思考","夜の一問","計算ひっかけ"],"summary":"昼3m登り夜2m落ちるカタツムリが深さ10mの井戸を出る日数を問う古典。差し引き1m/日で10日と早合点させ、8日目の昼に到達した時点で終わることに気づかせる。","illustration_scene":"夜、月明かりに照らされた古い石造りの井戸を真横から見た断面図。底の近くの壁に小さなカタツムリが一匹はりついている。井戸の上の夜空に大きな「?」。目盛り・数字・文字は描かない。人物は描かない。"}',
        '類型: 計算の早合点を突く古典(作者不詳・国際的に流布)。流布例: https://detail.chiebukuro.yahoo.co.jp/qa/question_detail/q1416591114 , https://sist8.com/30mi 。文面はオリジナルに書き下ろし(表現は書き直し済み)。', 1);
...
VALUES ((SELECT id FROM batch_sets WHERE set_code = 'logic-training-1'),
        (SELECT CONCAT('night-', LPAD(COALESCE(MAX(CAST(SUBSTRING_INDEX(t.content_key, '-', -1) AS UNSIGNED)), 0) + 1, 3, '0')) FROM (SELECT q.content_key FROM quiz_stock_items q JOIN batch_sets b ON b.id = q.set_id WHERE b.set_code = 'logic-training-1' AND q.content_key LIKE 'night-%') t),
        'L1', 'deep',
        '同じ長さの棒が6本ある。折ったり重ねたりせずに、6本だけで同じ大きさの正三角形を4つ作れ。どう並べる?',
        '立体にして三角錐(正四面体)を組む',
        '{"hook":"爪楊枝があれば試せるぞ","hint":"机から持ち上げてみろ!","question":"同じ長さの棒が6本ある。折ったり重ねたりせずに、6本だけで同じ大きさの正三角形を4つ作れ。どう並べる?","answer":"立体にして三角錐(正四面体)を組む","explanation":"平面では棒6本で正三角形は2つが限界。3本で底の三角形を作り、残り3本を1つの頂点に向けて立てると三角錐になり、4つの面すべてが正三角形になる。","coach_comment":"発想も立ち上がれ!","tags":["水平思考","夜の一問","図形パズル"],"summary":"棒6本で正三角形を4つ作る古典パズル。平面の思い込みを捨て、立体の三角錐(正四面体)を組めば4面すべてが正三角形になる。","illustration_scene":"夜の書斎。デスクランプの明かりの下、机の上に棒5本で平らな正三角形が2つ並べてあり、1本だけ余って転がっている。机の上の宙に大きな「?」。立体は作らない。文字・数字は描かない。人物は描かない。"}',
        '類型: 立体への発想転換(作者不詳の定番マッチ棒パズル)。流布例: https://detail.chiebukuro.yahoo.co.jp/qa/question_detail/q10102992220 , http://pzl.jp/q25 , https://sansu-seijin.jp/blog/archives/573 。文面はオリジナルに書き下ろし(表現は書き直し済み)。', 1);
