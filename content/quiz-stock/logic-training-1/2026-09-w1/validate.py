# 2026-09-w1 補充: stock_items.py のフィールド仕様検証
...
# C35: 容量パズルの全手順(5L→3L→空ける→2L を移す→5L→3L 満杯まで)を answer に書くため 30 字に収まらない。
# C37: 南京錠の往復手順(送る→錠を足して返送→自分の錠を外して再送→開ける)も同様。
# A34: 見て言う数列の解説は 1→11→21→1211→111221→312211 の全段を省かず書く(2026-09-04 ユーザー指示)。
#      explanation もキャプション専用フィールド(版面に出ない)のため answer と同じ理由で緩和する。
OVERRIDES = {("C35", "answer"): 60, ("C37", "answer"): 45, ("A34", "explanation"): 120}
errors = []
