# 2026-09-w3 補充: stock_items.py から review.md(人間レビュー用)と insert_quiz_stock.sql(投入用)を生成
import json
import sys

sys.path.insert(0, __file__.rsplit("/", 1)[0])
from content_fields import content_fields_of
...
# ---------- review.md ----------
lines = [
    "# 2026-09-w3 問題ストック補充: 14 問レビューシート",
    "",
    "投入前の人間レビュー用(operation.html セクション 3 手順 3)。レビュー進捗はSTATUS.mdが正。今回の範囲は夜7問のチャットレビューまでで、全問承認後にコミット・pull --rebase・pushして終了する。投入・S3配置・Aurora更新は行わない。",
    "",
    "補充対象は **朝 = なぞなぞ・言葉あそび / 夜 = とんち・水平思考・ひっかけの古典** の 2 組 × 7 問(1 週間分)。昼(L3 / standard)は 16-4d で停止中のため補充しない。",
    "",
    "## 着手時の在庫(2026-09-20)",
    "",
    "- 朝(L1/light)= 未使用かつビルド済み 0 問 / 夜(L1/deep)= 0 問(すでに再利用に落ちている)。unbuilt は 0。不足 = 各 7 問を補充",
    "- 昼(L3/standard)= 未使用 7 問。停止中のため補充対象外(既存在庫は凍結・残置)",
    "",
    "## レビュー観点(operation.html 手順 3)",
    "",
    "1. **誤答チェック**: verify_logic.pyの機械検証は各実装モデルの範囲に限る。夜はC46の鎖の接続モデルを検証し、他6問は自動検証対象外。全問の難度・想定解の自然さ・道具の実現性・別解は人間レビューで確認する。全問の一意解を保証したものではない。",
    "2. **表現の独自性・出典**: 夜は古典の解法構造を採用し、日本語表現は書き直した。朝のA41・A45は承認済みの自作問題。直接確認したページと取得できなかったページはresearch.mdで区別する。",
    "3. **既存ストック・出題履歴との重複**: 2026-09-20にローカルの全5バッチの問題要旨を照合した。C45は2026-09-20のレビューで曹沖のゾウが却下され、つかえたトラックへ差し替えたため前バッチC41との水の近さは解消した。今回Auroraは再照合していないため、別途投入する直前に再確認が必要。",
    "4. **フォーマット適合**: 文字数上限・tags 数・出典 URL は validate.py で機械検証済み(全問適合)",
    "",
    "修正指示は「C42 の問題文を〜に」のように番号 + 項目名でお願いします。",
    "",
]
...
lines += [
    ...
    "- Aurora は Data API 経由。別途投入の承認を得て実施する。",
    "- 投入前に Aurora の `quiz_items` の summary 全件を取得し、既出題との重複がないか最終確認する",
    "- 投入直後は未ビルド件数の増分を確認し、ビルド・配置後に未使用ビルド済み在庫を確認する(operation.html セクション 3)",
...
for it in ITEMS:
    cf = content_fields_of(it)  # validate.py の実行時同等検証と同じ dict(I-033)
