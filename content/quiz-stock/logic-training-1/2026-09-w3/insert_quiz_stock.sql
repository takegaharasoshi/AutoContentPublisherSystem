...
VALUES ((SELECT id FROM batch_sets WHERE set_code = 'logic-training-1'),
        (SELECT CONCAT('night-', LPAD(COALESCE(MAX(CAST(SUBSTRING_INDEX(t.content_key, '-', -1) AS UNSIGNED)), 0) + 1, 3, '0')) FROM (SELECT q.content_key FROM quiz_stock_items q JOIN batch_sets b ON b.id = q.set_id WHERE b.set_code = 'logic-training-1' AND q.content_key LIKE 'night-%') t),
        'L1', 'deep',
        '四角い離れ小島を、同じ幅の堀が四角く囲んでいる。堀の幅は板より少し広い。板は2枚、釘も紐もない。島へ渡る方法は?',
        '2枚をTの字に組み、堀の角から島の角へ渡す',
        '{"hook":"板が足りないのに渡れるってさ!","hint":"まっすぐ渡ろうとするな!","question":"四角い離れ小島を、同じ幅の堀が四角く囲んでいる。堀の幅は板より少し広い。板は2枚、釘も紐もない。島へ渡る方法は?","answer":"2枚をTの字に組み、堀の角から島の角へ渡す","explanation":"まず外側の角へ1枚を斜めに渡す。その板の真ん中から島の角へもう1枚を架けると、2枚がTの字になって届く。まっすぐ架ける発想を捨てるのが鍵だ。","coach_comment":"1本の橋にこだわらなかったな!","tags":["水平思考","夜の一問","空間発想"],"summary":"四角い離れ小島を囲む堀を、幅より短い板2枚で渡る古典。外側の角へ1枚を斜めに渡し、その中点から島の角へもう1枚を架けてTの字に組む。堀幅は板の約1.06倍まで可能。","illustration_scene":"月夜の俯瞰。四角い離れ小島と、それを囲む一定の幅の堀が正方形に広がる。島の中央に小さな石の建物。堀の外側の岸に、まっすぐな木の板が2枚並べて置かれている。板は堀の幅よりわずかに短い。中央に大きな「?」。渡した板・斜めの板・Tの字に組んだ板・橋・人物・文字・数字は描かない。"}',
        '類型: 堀を2枚の板で渡る古典(Moat crossing)。流布例: https://iterativetangents.com/moat-crossing-puzzle/ (「an old riddle」として紹介・作者の記載なし) , https://richardwiseman.wordpress.com/2009/09/04/its-the-friday-puzzle-23/ (コメント欄で複数が well known problem / classic と言及)。2026-09-21 Claude が両本文を直接確認。原話の数値(10m の堀に 9.5m の板など)と城攻めの文脈は使わず、「板より少し広い堀」として書き直し済み。板の長さは堀幅の約0.943倍以上が必要で、verify_logic.py で検算している。', 1);
...
VALUES ((SELECT id FROM batch_sets WHERE set_code = 'logic-training-1'),
        (SELECT CONCAT('night-', LPAD(COALESCE(MAX(CAST(SUBSTRING_INDEX(t.content_key, '-', -1) AS UNSIGNED)), 0) + 1, 3, '0')) FROM (SELECT q.content_key FROM quiz_stock_items q JOIN batch_sets b ON b.id = q.set_id WHERE b.set_code = 'logic-training-1' AND q.content_key LIKE 'night-%') t),
        'L1', 'deep',
        '広げた新聞紙1枚の上に、大人2人が同時に立つ。普通なら手が届く距離だが、互いに手を伸ばしても触れられないようにしたい。紙は破らない。どうする?',
        'ドアの下に新聞紙を敷き、ドアを閉めて両側に立つ',
        '{"hook":"同じ紙の上なのに届かない!","hint":"紙をどこに置くかが勝負だ!","question":"広げた新聞紙1枚の上に、大人2人が同時に立つ。普通なら手が届く距離だが、互いに手を伸ばしても触れられないようにしたい。紙は破らない。どうする?","answer":"ドアの下に新聞紙を敷き、ドアを閉めて両側に立つ","explanation":"開いたドアの下へ新聞紙を差し込み、ドアを閉める。2人は同じ1枚の上に立ちながら、ドアに隔てられて手が届かない。紙ではなく部屋の方を使うのが鍵だ。","coach_comment":"紙より部屋の使い方だったな!","tags":["水平思考","夜の一問","盲点"],"summary":"新聞紙1枚の上に2人が立ちながら触れ合えない状態を、ドアの下へ紙を敷き両側に立つことで作る古典。紙の置き方ではなく、部屋という周囲の環境へ目を向けさせる。","illustration_scene":"夜の室内、天井の照明。板張りの床に新聞紙が1枚だけ広げて置かれている。少し離れた場所に大人2人の後ろ姿が立ち、まだ紙の上には乗っていない。上に大きな「?」。ドア・戸口・仕切り・顔・文字・数字は描かない。"}',
        '類型: 新聞紙1枚の上の2人(Newspaper Divider。作者不詳で広く流布するなぞなぞ)。流布例: https://www.riddles.com/37 (Anonymous 表記)。2026-09-21 Claude が本文を直接確認。braingle.com・riddleswiz.com・brainly.com にも同じ問題と解答が載っていることを検索結果で確認したが、403 や名前解決不可で本文の直接確認はできていない。原話の「兄妹げんかの罰」という設定は使わず、状況だけを示す日本語の出題へ書き直し済み。想定解であり、紙の強度などを機械検証したものではない。', 1);
