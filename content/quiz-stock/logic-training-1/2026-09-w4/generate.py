# 2026-09-w4 補充: stock_items.py から review.md(人間レビュー用)と insert_quiz_stock.sql(投入用)を生成
import json
import sys
...
# ---------- review.md ----------
lines = [
    "# 2026-09-w4 問題ストック補充: 14 問レビューシート",
    "",
    "投入前の人間レビュー用(operation.html セクション 3 手順 3)。レビュー進捗はSTATUS.mdが正。改修 R-3 後の初回バッチで、候補選別(candidates/2026-09-w4/)を通った OK のうち目標数だけを仕上げた。",
    "",
    "補充対象は **朝 = なぞなぞ・言葉あそび / 夜 = とんち・水平思考・ひっかけの古典** の 2 組 × 7 問(1 週間分)。昼(L3 / standard)は 16-4d で停止中のため補充しない。",
    "",
    "## 着手時の在庫(2026-09-23)",
    "",
    "- 朝(L1/light)= 未使用かつビルド済み 4 問 / 夜(L1/deep)= 5 問。unbuilt は 0。閾値 7 未満のため各 7 問を補充",
    "- 昼(L3/standard)= 未使用 7 問。停止中のため補充対象外(既存在庫は凍結・残置)",
    "",
    "## レビュー観点(operation.html 手順 3)",
    "",
    "1. **誤答チェック**: verify_logic.py は逆さ読み・時計・頭文字・字形・縄ばしご・バスの向きを検証した。A49・C49・C51・C53・C54・C55 は自動検証対象外。全問の難度・想定解の自然さ・別解は人間レビューで確認する。",
    "2. **表現の独自性・出典**: 全問が folklore 級の類型で、流布例 URL は候補段階で直接確認済み。日本語表現は書き下ろし。",
    "3. **既存ストック・出題履歴との重複**: 候補段階でローカルの全 6 バッチの問題文と照合した。Aurora の summary 全件突合は投入直前に行う。",
    "4. **フォーマット適合**: 文字数上限・tags 数・出典 URL は validate.py で機械検証済み(全問適合)",
    "",
    "修正指示は「C49 の問題文を〜に」のように番号 + 項目名でお願いします。",
    "",
]
