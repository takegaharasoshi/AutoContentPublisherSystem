...
def review_markdown_item(item: dict[str, Any], data: dict[str, Any]) -> str:
    ...
    has_narration = False
    for duration, label, keys in (
        ("20s", "20 秒版", validate.NARRATION_KEYS_20S),
        ("30s", "30 秒版", validate.NARRATION_KEYS_30S),
    ):
        if duration not in item["narration"]:
            continue
        if has_narration:
            lines.append("")
        lines.extend([f"### {label}ナレーション", ""])
        lines.extend(
            f"- `{cue}`: {item['narration'][duration][cue]}"
            for cue in keys
        )
        has_narration = True
    lines.extend(
...
<title>都道府県ランキングストック 初期投入レビュー（第 3 バッチ 16 件 / 17-7）</title>
<style>
:root {{
  --page: #f6f4ef;
  --surface: #fffdf8;
...
<main>
<header class="lead">
<h1>都道府県ランキングストック 初期投入レビュー（第 3 バッチ 16 件 / 17-7）</h1>
...
def build_sql(items_and_data: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    Returns:
        Complete SQL text.
    """
    header = """-- 都道府県ランキングストック初期投入 第 3 バッチ（16 件。レビュー承認後に実行）
