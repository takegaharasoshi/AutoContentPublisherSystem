...
最終更新: 2026-09-19。**21-4a-3 ② の 1 問目 U15 が人間ゲート通過**（「画面より先に聞こえたホームラン」misdirection / 難易度 4 / content_key `006-cheer-before-screen`。ストックは 6 問）。コアゲートは 3 巡（story 案 2 件〔カーテンの留守宅防犯・砂時計の故郷の砂〕が NG〔理由の指摘なし。Claude の仮説「情緒への反転は落差不足」のスキル追記は不採用 = 理由は別〕→ 4 回目の抽選「ラジオ」で承認）。**次は 21-4a-3 ② の 2 問目: U16（content_key 007）**（新しいセッションで `/step 21-4a-3` から着手する。開発レーンは常に 1 セッションのみ。作問前に本ファイルの「② への申し送り」を読むこと）。
...
機械検査（U15 追加時点）: validate 全 6 問 OK（警告 0）・leak_count 116 想定質問で核心語漏れ 0・冒頭語不一致 0・dry-run 6 件 OK。

## 21-4a-3 ② への申し送り（作問の勘所。詳細は SKILL.md と開発記録の各巡）

- **型・難易度の偏り**: 現 6 問は misdirection 6 / story 0、難易度 4 が 5 問・2 が 1 問。設計上の目安（story と misdirection を混ぜる）に対し story が 0 のため、**U16〜U19 の語選び（工程 1）で story 型を引き続き狙う**。既定難易度は 4（A 型手がかり 0 個・消去法動線。9 巡目のユーザー指示）
...
- [ ] 21-4a-3 ②: スキルで U15〜U19（content_key 006〜010）を 1 問 1 /goal で作問・人間ゲート反復（スキルのブラッシュアップ継続）。**U15 通過（2026-09-19）**、残り U16〜U19
