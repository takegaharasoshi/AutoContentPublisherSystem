...
（動画レンダラーの Remotion 載せ替え）の成果物です。運用手順の正は
...
| `../build.py` | 対象抽出、props 生成、レンダリング、音声ミックス、台帳更新の入口 |
| `scripts/fetch_assets.py` | コーチ立ち絵を S3 から取得し 4 表情共通の矩形でトリミング |
| `scripts/mix_audio.py` | 現行版と同じ ffmpeg フィルタで BGM + SE を載せ、レビュー用スチルを抽出 |
| `scripts/spec.py` | Python 側の尺・SE 時刻・音量・スチル位置の単一ソース |
...
| 置き場 | 中身 | 取得元 |
|---|---|---|
| `public/fonts/` | `NotoSansJP-Regular.otf` / `NotoSansJP-Bold.otf` | `../build.py` が pref-ranking-1 の `public/fonts/` から複製 |
| `public/coach/` | コーチ立ち絵 4 表情 | S3 `assets/logic-training-1/coach_*.png`（`fetch_assets.py`） |
| `public/illustrations/` | 問題ごとの情景イラスト | `../work/illustrations/<id>.png`（`../build.py`） |
...
```bash
# リポジトリルートで初回のみ（pref-ranking-1 側で作成済みならそのまま使える）
...
S3_BUCKET_NAME=<bucket> python build.py --item <quiz_stock_items.id>
```

`work/silent/<id>.mp4`（映像のみ）、`work/videos/<id>.mp4`（音声込み）、
`work/cuts/<id>_*.png`（レビュー用スチル）が出ます。新規ビルドで動画完成後にだけ
BGM の LRU を更新し、`--rebuild` では既存 BGM を引き継いで LRU を更新しません。
