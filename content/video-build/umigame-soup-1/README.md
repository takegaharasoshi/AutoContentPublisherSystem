...
python3 background_sheet.py --stills   # 各コマの静止画も今の文言・背景で書き出して並べる（Remotion renderStill・1 問 10 秒ほど）

# 2. BGM を用意する。初回は PoC の 24 秒素材を暫定 track01 として台帳へ登録する
python3 prepare_bgm.py --init-provisional
python3 prepare_bgm.py

# 3. ナレーション（Irodori-TTS・ローカル GPU）を合成して 24 秒動画をビルドする
python3 build.py
python3 build.py --retake-tts --content-key <key>   # 予算超過・読み間違いの問を取り直す（1 問 3 回まで）
...
### ナレーション（21-5c〔2026-09-27〕で Irodori-TTS・声 G4 のクローンへ切替）

- **既定は Irodori**（`scripts/narration_irodori.py`・`build.py --tts irodori`）。ローカル GPU で
  `~/tools/Irodori-TTS`（環境変数 `IRODORI_DIR` で変更可。環境構築と決定の経緯は `poc/irodori-tts/README.md`）を
  subprocess で呼ぶ。モデル・精度（BF16 必須）・seed・長さ・参照音声の場所は `assets/design.json` の `narration` 節が正、
  声は `assets/voice/kamerock-g4/*.wav`（6 本・名前順）のクローン。API キー・AWS の資格情報は不要。
- 問題文とルール文を **1 回の合成**で読ませ（別々に作るとルール文で口調が変わる）、文字数比に最も近い
  0.3 秒以上の無音で切り分けて前後の無音を削る。話速調整はせず `--seconds 20.0` で長さを指定する。
  2 cue + 間 1.2 秒が 21 秒を超えたら seed を進めて `--seconds` を 0.5 秒ずつ縮めて取り直す（1 回のビルドで最大 3 テイク）。
  聞いて気になった問は `--retake-tts` で取り直す（1 問 3 回まで）。1 テイク約 45 秒。
- `work/narration/<key>/narration.json` に model・参照音声（sha256）・precision・seed・seconds・切断位置
  （`split_at_seconds`）・試したテイク（`takes`）・通算の合成回数（`attempt`）が残る。設定が一致して予算内なら
  キャッシュを使い合成しない。Irodori のログは同じディレクトリの `irodori.log`。
- `infer.py` が失敗したら（GPU・環境の問題）ビルドはその場で中断する。
- Gemini 版は `build.py --tts gemini`（`scripts/narration_gemini.py`・設定は design.json の `narration_gemini` 節。
  日次上限 100 リクエストのため本番では使わない）、Polly 版は `--tts polly`（Takumi 125%。21-5a / 21-5b の初版 14 本）、
  VOICEVOX 版は `scripts/narration.py`（PoC 用 CLI）として残す。声の比較 PoC は `poc/gemini-tts/`・`poc/irodori-tts/`。
...
├── scripts/
│   ├── narration_irodori.py  # 本番の TTS（Irodori-TTS・声 G4 のクローン。1 回で合成 → 無音で切り分け → 予算検査。21-5c）
│   ├── narration_gemini.py   # Gemini 3.8 Flash TTS のアダプタ（build.py --tts gemini。日次上限のため本番では使わない）
...
### 人間ゲート通過（2026-09-04・ユーザー承認）

第 3 稿をもって 21-2 の人間ゲートを通過した。3 点の判断はいずれも承認。

| 判断項目 | 結果 |
|---|---|
| リールフォーマットの go / no-go | **go**（24 秒・9:16・ループ前提。この版面を umigame-prebuilt の基準とする） |
| ストック 1 件の素材項目 | **下表で確定**（21-3 のセット別設計書・21-4a の `stock_items.py` / `validate.py` はこの表を正とする） |
| Remotion Skills を本セットの標準ツーリングとするか | **継続**（`.agents/skills` 正本 + `.claude/skills` リンクを維持し、`npx skills update` で更新。既存 2 セットへの遡及適用は本フェーズの対象外） |

#### 確定した素材項目（1 ストック = 1 問）

| 項目 | 用途 | 制約（第 2・3 稿の実測） |
|---|---|---|
| `problem_text` 問題文 | 版面の問題カード | 78〜85 字。ナレーション実測長で最終判定（`validate.py` で検査） |
| `truth` 真相 | AI 出題者の判定根拠（版面には出さない） | — |
| `fact_sheet` 確定事実シート | 「はい / いいえ / 関係ない」の判定根拠 | 8〜12 件 |
| `expected_questions` 想定質問と期待回答 | プローブテスト（21-4a） | 15〜20 件 |
| `hook` フック文 | つかみ帯 | 12 字前後（74px で 1 行） |
| `rule_text` ルール帯 | 常時表示の遊び方 | 45 字前後（34px で 2 行） |
| `narration_cue.problem` / `.rule` | 冒頭ナレーション（表示文とは別に読点を整えた読み上げ用） | 2 cue 合計 + 間 1.2 秒で 21 秒以内 |
| `play_example` プレイ例 | 下部の吹き出し 3 往復（質問 → 質問 + 返答） | 質問 16 字 / 返答 17 字以内（1 行）。**3 往復のうち 1 つは「はい」**（出題者が喜ぶポーズに切り替わる） |
| `master_lines.intro` / `.outro` | 出題者の導入・締め | 各 17 字以内 |
| `jr_lines.outro` | Jr. の締め（いいね・フォロー） | セット固定文でよい |
| `illustration_prompt` イラスト作成用プロンプト | 背景 1 枚（imagegen） | 画風固定行 + 9:16 + 文字禁止 + **真相の手がかりを描かない** |
| `caption` キャプション + ハッシュタグ | 投稿本文 | `#AIart` 必須。VOICEVOX クレジットは Polly 採用により不要 |
| `source_note` 出典・オリジナル性メモ | 証跡 | 完全オリジナル宣言（PoC の古典題材は例外） |
| `difficulty` 難易度 | 運用の目安 | 1〜5 |

キャラクター（カメロック・Jr.）と BGM・SE はセット固定の共有資産で、ストック単位の項目ではない。

#### 21-3 以降への申し送り

- **BGM は 24 秒ちょうどのトラックとして調達する**（本 PoC は 20 秒トラックのループのため 20 秒地点につなぎ目がある）。`prepare_bgm.py`（pref-ranking-1）の 30 秒固定を 24 秒版として用意する。
- TTS は Amazon Polly Neural（Takumi・125%）。アダプタは差し替え可能な形にする（`scripts/narration_polly.py`）。
- 版面の座標・ビート・効果は `remotion/src/{timeline.ts,UmigameReel.tsx}` が実装の正。設計書には構成と不変条件（720 フレーム・周期は約数・スナップ切替・ループ継ぎ目）を書く。
