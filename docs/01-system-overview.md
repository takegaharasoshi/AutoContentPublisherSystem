...
2. Step Functions が ECS Fargate タスクを起動
3. Fargate タスクが Nano Banana Pro（Gemini 3 Pro 画像 API）を呼び出し、画像を生成
4. 生成した画像を S3 に保存
5. メタ情報を Aurora Serverless v2 に登録

### 2.2 SNS 投稿バッチ

1. EventBridge Scheduler が定期的に Step Functions を起動
2. Step Functions が ECS Fargate タスクを起動
3. Fargate タスクが DB から投稿対象を取得
4. S3 の画像に対して Presigned URL を発行（Instagram Graph API は公開到達可能な URL を要求するため）
5. Instagram Graph API に Presigned URL を渡して投稿
6. 投稿結果を DB に記録
...
- 画像生成バッチと SNS 投稿バッチを 1 セットとし、将来的に複数セットを運用する
  ...
  - スケジュール定義のマスタは IaC（CDK）、DB の `batch_schedules` テーブルは運用参照用コピーとする
