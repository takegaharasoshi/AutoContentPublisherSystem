...
- **リトライ設計**:
  ...
  ```
  > Aurora Serverless v2 のコールドスタート（自動一時停止からの再開）は 25 秒〜数分かかる場合があるため、最大約 510 秒の待機時間を確保する。
...
- Step Functions ステートマシンはバッチ種別ごとに 1 つとする（image-generation-sfn, sns-posting-sfn）。セット数が増加し、スケジュール時刻からの遅延が蓄積する場合は、ステートマシンのセットごとの分離を検討する
...
> **StartSnsPostBatch のエラーハンドリング方針**: SNS 投稿 Step Functions の `StartExecution` 呼び出しには Retry/Catch を設定する。Retry 後も起動に失敗した場合は `NotifySnsPostStartFailure`（Task ステート）に遷移し、CloudWatch カスタムメトリクス `ACPS/SnsPostStartFailureCount` を発行した後、`ImageBatchSucceeded`（Succeed ステート）に遷移する。画像生成 ECS タスクは既に成功しているため、SNS 投稿の起動失敗のみで画像生成ワークフロー全体を失敗にはしない。SNS 投稿が起動されなかったことは MonitoringStack のカスタムメトリクスアラームで検知し、手動で sns-posting-sfn を直接起動する。
...
| 2 | Step Functions Standard にはステートマシンの同時実行数を制限するネイティブ機能がない | 手動実行とスケジュール実行が重なった場合、同一ステートマシンが並行実行される。不要な外部 API コール（画像生成 API の二重呼び出し等）が発生する可能性がある | DB の UNIQUE 制約と親行ロックにより、データの整合性は保証される。EventBridge Scheduler の実行間隔をバッチ想定実行時間より十分長く設定することで通常運用では回避可能 |
