...
> **順次実行の設計方針**: 画像生成完了後に SNS 投稿を自動実行することで、生成直後の画像が確実に投稿される。SNS 投稿 Step Functions（sns-posting-sfn）は独立したステートマシンとして存在するため、手動での再投稿や投稿のみの単独実行も引き続き可能。EventBridge Scheduler は画像生成 Step Functions のみに設定し、SNS 投稿用のスケジューラは不要。
...
- **配置**: Public Subnet（`assignPublicIp=ENABLED`）。Aurora（Isolated Subnet）へ VPC 内部ルーティングで接続する。Secrets Manager へはパブリック IP 経由でアクセスする
- **リトライ設計**:
  ```
  最大リトライ回数: 5
...
- **成功時**: 終了コード 0 で終了し、Step Functions が次のステート（バッチ ECS RunTask）へ遷移する
- **失敗時**: 終了コード 1 で終了し、Step Functions の Retry/Catch でハンドリングする
...
| レイヤー | ハンドリング方式 |
|---|---|
| ECS タスク（DB 準備確認） | DB 接続リトライ（指数バックオフ、最大 5 回）。失敗時は終了コード 1 で終了 |
| Python コード（バッチ） | try/except で例外捕捉、ログ出力、適切な終了コードを返す（DB 接続リトライは不要。DB は利用可能前提） |
| Step Functions | Retry（リトライ可能エラー）、Catch（リトライ不可エラー）。WaitForDbReady ステートの失敗も Retry/Catch で処理 |
| EventBridge | Step Functions の起動失敗は CloudWatch で検知 |

### 1.6 同時実行制御

- Step Functions ステートマシンごとに**同時実行数を 1 に制限する**
- これにより、同一セットの手動実行とスケジュール実行が重なった場合の二重投稿・二重生成を防止する
- 同時実行数の上限に達した場合、後続の実行はキューイングされ、先行実行の完了後に順次実行される
- DB レベルの排他制御（親行ロック + UNIQUE 制約）との多層防御により、万一の並行実行時にもデータ整合性を保証する

> **スケーラビリティに関する注記**: 現在のステートマシン構成（バッチ種別ごとに 1 つ、全セット共有）では、同時実行数 1 の制約により複数セットの実行がキューイングされる。セット数が少ない初期段階では問題ないが、セット数の増加によりスケジュール時刻からの遅延が蓄積する場合は、ステートマシンをセットごとに分離する対応を検討する。

### 1.7 複数セット運用方針

- Step Functions ステートマシンはバッチ種別ごとに 1 つとする（image-generation-sfn, sns-posting-sfn）。セット数が増加した場合、同時実行数 1 の制約により各セットの実行がキューイングされ、スケジュール時刻からの遅延が蓄積する可能性がある。セット数が増えてスケジュール遅延が問題になった場合は、ステートマシンのセットごとの分離を検討する
- EventBridge Scheduler はセットごとに作成し、画像生成 Step Functions をターゲットとして `set_code` を渡す（SNS 投稿用のスケジューラは不要。画像生成完了後に自動起動される）
...
- **S3 orphan 対応**: S3 Lifecycle Policy（30 日で自動削除）により、孤立ファイルを含む全オブジェクトが自動クリーンアップされる。SNS 投稿は通常数日以内に完了するため、30 日のバッファで運用上の問題はない
...
```json
{
  ...
  "States": {
    ...
    "SnsPostStartFailed": {
      "Type": "Succeed",
      "Comment": "画像生成は成功済みのため、SNS 投稿 SFN の起動失敗時もワークフローは成功終了とする。SNS 投稿が起動されなかったことは監視で検知し、手動で sns-posting-sfn を起動する"
    },
    "HandleError": {
      "Type": "Fail",
...
> **StartSnsPostBatch のエラーハンドリング方針**: SNS 投稿 Step Functions の `StartExecution` 呼び出しには Retry/Catch を設定する。Retry 後も起動に失敗した場合は `SnsPostStartFailed`（Succeed ステート）に遷移し、画像生成 Step Functions 自体は成功終了とする。画像生成 ECS タスクは既に成功しているため、SNS 投稿の起動失敗のみで画像生成ワークフロー全体を失敗にはしない。SNS 投稿が起動されなかったことは MonitoringStack の監視で検知し、手動で sns-posting-sfn を直接起動する。
...
Retry/Catch 付き ECS RunTask の構成。最初のステートとして DB 準備確認 ECS タスク（WaitForDbReady）を実行する。画像生成 Step Functions から `StartExecution` で起動されるほか、手動での単独起動も可能。
...
> **注記**: sns-posting-sfn は画像生成 Step Functions から `startExecution`（非同期）で起動される。画像生成 Step Functions は SNS 投稿の完了を待たずに終了する。SNS 投稿の成否は sns-posting-sfn 自身の実行ステータスおよび監視で確認する。
...
| 観点 | 制約 | 備考 |
|---|---|---|
| Instagram Graph API レートリミット | アカウントあたり 200 リクエスト/時間（公式ドキュメント参照） | 1 投稿あたりコンテナ作成 + パブリッシュの 2 API コールが必要。50 件 × 2 = 100 リクエストでリミットの 50% |
| ECS タスクタイムアウト | Step Functions のタイムアウト設定に依存 | 50 件の投稿処理が設定タイムアウト内に完了する必要がある |
| Presigned URL 有効期限 | 1 時間 | バッチ開始時に発行した URL が処理完了前に期限切れにならないこと |
