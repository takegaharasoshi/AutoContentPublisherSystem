...
Aurora Serverless v2 の自動一時停止からの再開に対応するため、各 Step Functions ワークフローの最初のステートとして DB 準備確認 Lambda を実行する。

- **Lambda 関数**: FoundationStack で定義（db-readiness-check）。両ワークフローで共有する
- **配置**: VPC 内（Public Subnet）。Aurora（Isolated Subnet）へ VPC 内部ルーティングで接続する
- **リトライ設計**:
  ```
  最大リトライ回数: 5
...
- **成功時**: 正常レスポンスを返し、Step Functions が次のステート（ECS RunTask）へ遷移する
- **失敗時**: 例外を送出し、Step Functions の Retry/Catch でハンドリングする
...
| レイヤー | ハンドリング方式 |
|---|---|
| Lambda（DB 準備確認） | DB 接続リトライ（指数バックオフ、最大 5 回）。失敗時は例外送出 |
...
```json
{
  ...
  "States": {
    "WaitForDbReady": {
      ...
      "Parameters": {
        "FunctionName": "${DbReadinessCheckLambdaArn}",
        "Payload": {}
      },
      "Retry": [
        {
...
Retry/Catch 付き ECS RunTask の構成。最初のステートとして DB 準備確認 Lambda（WaitForDbReady）を実行する。画像生成 Step Functions から `StartExecution` で起動されるほか、手動での単独起動も可能。
