...
2. `cdk deploy <StackName>` でインフラを更新する
3. **ImageBatchStack または SnsPostBatchStack に変更が含まれる場合**、対応するサービスパイプラインを手動実行する
   - AWS Console → CodePipeline → 該当パイプラインの「変更をリリースする」を実行
   - CodeBuild が最新イメージで新 revision を登録し、CDK で変更されたロール・環境変数が反映される
4. Step Functions が次回実行時に最新 revision を使用することを確認する

### 6.2 パイプライン手動実行が必要なケース

| 変更内容 | パイプライン手動実行 |
|---|---|
| FoundationStack のみ（VPC、S3、Aurora 等） | 不要（タスク定義に影響しない） |
