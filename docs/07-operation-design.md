...
5. SNS 投稿 Step Functions が DB 準備確認 ECS タスクを実行（WaitForDbReady ステート）
...
| エラー種別 | 対応 |
|---|---|
| DB 接続失敗 | Step Functions の WaitForDbReady ステート（ECS タスク）で事前確認済み。バッチ ECS タスク実行中に DB 接続が切れた場合はログ出力後、終了コード 1 で終了 → Step Functions Retry |
