...
> **SNS 投稿起動の冪等性**: image-generation-sfn から sns-posting-sfn を起動する際は、親の `$$.Execution.Name` を子実行の `Name` として使用する。これにより `StartExecution` の Retry 時も同一要求として扱われ、二重起動のリスクを抑制できる。既に同名の子実行が作成済みの場合は成功扱いとする。詳細は [specs/workflow.md](../specs/workflow.md) を参照。
...
- 構造化ログ（JSON 形式）を推奨
- API キー、アクセストークン、Authorization ヘッダー、Presigned URL、Secret 値はログに出力しない
- 外部 API レスポンスをログや DB に保存する場合は、保存前に認証情報・署名付き URL・個人情報に該当し得る値をマスキングする
...
| EventBridge Scheduler | Step Functions の起動失敗は Scheduler の Retry / DLQ と CloudWatch Alarm で検知する。監視リソースは [specs/workflow.md](../specs/workflow.md) を参照 |
...
| パブリッシュ API | container_id 保存済み、投稿結果が未確定 | container_id ≠ NULL、platform_post_id = NULL → 同じ container_id でパブリッシュ再試行。成功時は platform_post_id を保存して success。重複 publish など「投稿済みの可能性があるが post_id を確定できない」応答、または publish 要求送信後のタイムアウトは `published_unconfirmed` に更新して自動再試行を止める |
| platform_post_id の DB 保存 | 投稿済みだが status が pending のまま残る可能性がある | 次回実行で同じ container_id の publish を再試行し、success または `published_unconfirmed` に収束させる。新規 attempt は作成しない |
| 投稿失敗（status=failed） | 前回バッチで投稿未完了として失敗した | 次回バッチ実行時に success / `published_unconfirmed` が存在しない組として再検出 → 新規 pending レコード（attempt_number インクリメント）で再試行。新規 attempt 作成前に retry 上限（max_post_retries）を確認し、上限超過時はスキップ |

> **注記**: `status='failed'` のレコードは復旧対象ではなく、履歴として保持される。再試行は常に新しい `pending` レコードの作成により行われる。ただし `status='published_unconfirmed'` は「投稿済みの可能性を否定できない」終端状態として扱い、自動再試行・新規 attempt 作成の対象外とする。運用者が Instagram 側を確認し、投稿済みなら `success` 相当の補正、未投稿なら原因調査の上で手動再試行を判断する。
...
- **二重投稿防止**: 同一 container_id の再 publish で投稿済みの可能性が検出された場合は `published_unconfirmed` に遷移させ、新しい container を作成しない。これにより「publish 成功後に DB 更新だけ失敗した」ケースでの二重投稿を防ぐ
...
- `attempt_count` は同一 `execution_arn` / `batch_type` 内で ECS タスクが起動された回数を表す。初回 INSERT 時は 1 とし、Step Functions Retry による再起動時は 1 加算する
...
開始（※ DB 準備確認は Step Functions の WaitForDbReady ステートで完了済み。DB は利用可能前提）
  ...
  │       - `batch_sets.is_active=0` の場合はスキップして正常終了する
  ...
  │         - API レスポンス内の image part を抽出する
  │         - image part が 0 件の場合は画像生成失敗として終了コード 1 で終了する
  │         - image part が複数件ある場合、初期実装では最初の 1 件のみ保存し、2 件目以降は WARNING ログを出力して破棄する
...
> **複数画像レスポンスの扱い**: 初期実装では `prompt_config` 1 件につき保存画像 1 件とする。複数画像を保存する要件が出た場合は、`generated_images` に `output_index` を追加し、冪等性 UNIQUE 制約を `(set_id, prompt_config_id, scheduled_at, output_index)` に変更する。
...
開始（※ DB 準備確認は Step Functions の WaitForDbReady ステートで完了済み。DB は利用可能前提）
  ...
  │         post_records に status='success' または status='published_unconfirmed' のレコードが存在しない (image, account) の組
  │       - failed のみ存在する組も対象に含まれる（終端状態がないため）
  ...
  │       - `status='success'` が存在する場合はスキップする
  │       - `status='published_unconfirmed'` が存在する場合は自動再試行せずスキップし、運用確認が必要な旨を WARNING ログに出力する
  │       - pending レコードが存在する場合（同一 attempt の中断復旧）:
  ...
  │         container_id を渡して投稿
  │       - API 成功時は platform_post_id を取得し、ステップ 6 で success として記録する
  │       - API が明確に投稿未完了の失敗を返した場合のみ pending レコードの status を failed に更新する（次回バッチ実行時に再試行される、上限まで）
  │       - API が「同一 container_id は publish 済み」等の重複 publish 応答を返した場合、または publish 要求送信後にタイムアウト・接続断で結果不明になった場合は、status を published_unconfirmed に更新し、新規 attempt を作成しない
  │
  ├── 6. 投稿結果を DB に記録
...
- 投稿対象の判定は `post_records` テーブルの `status='success'` または `status='published_unconfirmed'` レコードの有無で行う
- 各 (`generated_image_id`, `sns_account_id`) の組について、`status='success'` または `status='published_unconfirmed'` のレコードが存在する場合はスキップする
- **排他制御（親行ロック方式）**: 投稿対象の `generated_images` レコードを `SELECT ... FOR UPDATE` でロックし（親行は必ず存在するため、ロック対象が 0 件になる問題を回避）、ロック取得後に `post_records` から `status='success'` または `status='published_unconfirmed'` の有無を確認してから `pending` を挿入する。UNIQUE 制約 `(generated_image_id, sns_account_id, attempt_number)` により、万一並行 INSERT が発生しても DB レベルで重複を防止する（DuplicateKeyError はアプリ側で捕捉してスキップ）
...
- `published_unconfirmed` は投稿済みの可能性があるため、自動再投稿の対象外とする
...
| 5 | `batch_sets.max_post_retries` 超過後は自動投稿されない | 長期間放置すると S3 Lifecycle で画像が削除され、当該画像の投稿が恒久的に不能になるリスクがある | SNS 投稿バッチが `PostRetryExceededCount` カスタムメトリクスを発行し、MonitoringStack のアラームで検知する。通知後は原因調査、手動補正、再生成のいずれかで対応する |
...
| 7 | `published_unconfirmed` は自動復旧しない | 投稿済みか未投稿かを API レスポンスだけで確定できない場合、運用確認まで当該画像・アカウントの投稿が停止する | 二重投稿回避を優先する。運用者が Instagram 側を確認し、投稿済みなら DB を補正、未投稿なら `post_records` を確認した上で手動再実行する |
