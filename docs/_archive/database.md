...
> **一意性制約なしの理由**: `batch_execution_logs` にはテーブルレベルの一意性制約を設けない。`execution_arn` が NULL 許容のため単純な UNIQUE 制約は不適合であり、同一実行での二重 INSERT はアプリケーション側で防御する（開始時に 1 回だけ INSERT し、以降は UPDATE のみ）。詳細は [design/batch.md](../design/batch.md) セクション 1.9 を参照。
