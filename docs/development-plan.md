...
| 16-3b 事前生成転換の設計 Fix | **Fable 5** / デフォルト | — |
| 16-3c 事前生成方式の実装 | Opus 5 / high（指示書・diff レビュー。方式契約に関わる判断で詰まったら Fable 5 へ切替） | terra / high（新 generator は小型・ビルドツーリングは既存レンダラー流用） |
| 16-3d 初期 42 問の動画ビルド・レビュー・配置 | Opus 5 / high（ビルド運用の進行・レビュー指摘の反映） | イラスト一括生成のみ委譲（imagegen。採否判断は人間レビュー） |
...
>
> **16-3 完了後の仕様変更（2026-08-08 ユーザー決定）: 動画の事前生成方式へ転換**。16-3 の実 API 品質確認（看板文字・パネル数字の混入 = 情景起因でプロンプト禁止では抑えきれない）と Codex 組み込み imagegen の比較（画風指定でフラット統一 + 禁止事項全遵守 = 品質面最良・サブスク内で API 課金なし。[development-log.md](development-log.md) の 16-3 (8)）を受け、イラスト〜動画を**事前にローカルでビルドして全数人間レビューのうえ S3 に作り置き**し、実行時は事前動画を返す軽量方式へ差し替える。狙い: ①イラスト API 費のゼロ化 ②投稿される完成形をそのまま事前検品できる品質担保 ③実行時の失敗面が投稿のみに縮小（8/4 型障害の構造的解消）。生成基盤インフラ（SFN・Scheduler・ECS）は fantasy-animals-1 が使い続けるため不変。これに伴い 16-3b〜16-3d を追加し 16-4 の切替内容を改訂した。
...
- [x] **16-2b** 方式設計書の改訂（16-2 レビュー中のユーザー決定の正式反映）
  - 内容: 16-2 の執筆・レビュー過程で確定した以下のユーザー決定を方式設計書（generators/gpt-quiz-multicut.html）ほかへ正式反映する（16-3 の実装はこの改訂後の設計書を正とする）。**(1) カット構成の変更**: カット 2「問題」を「考える」（固定文「止めてじっくり〜」）へ、カット 3 カウントダウンへストックの `hint`（20 字・動的）を吹き出し表示 — 全問に `hint` フィールドを追加済み（2026-08-05 頃決定）**(2) L3 の次回補充方針**（2026-08-07 決定。詳細はスキル `/quiz-stock-replenish` の昼 L3 節）: 捻った分解型（非直接的な仮定を最低 1 つ経由）を標準・解説は 5 ステップ構造（前提 → ベース → 分解 → 計算 → 実勢チェック）・`explanation` 上限を L3 のみ 240 字程度へ拡張（キャプション専用フィールドのため版面影響なし。16-2 の初期 42 問は承認済みのため置き換えない）。validate.py の上限追随は次回補充時 **(3) ストック資材置き場の運用ルール化**（2026-08-07 の `content/` 新設の設計書反映）: 資材は `content/quiz-stock/<set_code>/<バッチ名>/` で git 管理・バッチ単位に自己完結・投入完了時にコミット・`review.md` は派生物で gitignore、を [operation.html](app/operation.html) セクション 3 に明文化する（リポジトリ構成としては CLAUDE.md 反映済み。運用ルールの正は設計書のため）
  - 確認条件: 方式設計書のカット構成・フィールド仕様が上記と一致し、16-3 の実装指示が書ける水準になっている。operation.html セクション 3 に資材置き場の運用ルールが記載されている
  - 備考: 2026-08-07 完了（Fable 5）。方式設計書 8.1 のカット構成を「つかみ 4 → 考える 4〔固定文を前倒し〕→ カウントダウン 5〔吹き出し = `hint` 動的〕→ 誘導 3」へ改訂（decision 新設。吹き出しは動的 2 + 固定文 2 へ）、セクション 5 に `hint` 行追加 + `explanation` 上限を L3 のみ 240 字へ拡張（L3 捻った分解型・5 ステップ解説の decision 新設。validate.py の追随は次回補充時）、operation.html セクション 3 に資材置き場 `content/quiz-stock/<set_code>/<バッチ名>/` の運用ルール 4 点を明文化。**コーチ表情はカット 2 = think・カット 3 = question へ入れ替え**（ユーザー決定外の 16-2b 整理として decision に明記）。セット別設計書・data-model.html・スキル `/quiz-stock-replenish` の暫定注記も追随。詳細は [development-log.md](development-log.md) の 16-2b を参照
- [x] **16-3** 生成・投稿パイプラインの改修（実装）
  ...
  - 備考: 2026-08-07 完了（実装 = Codex `gpt-5.6-sol` / high 委譲・指示書 + レビュー + blocker 修正 + 動画確認 = Fable 5）。設計どおり実装され pytest 全パス（image-batch 99 件 + sns-post-batch 108 件。ローカル MySQL E2E 込み）。レビューで blocker 1 件（カウントダウンバッジがカード右端基準で Instagram セーフエリアを約 40px 逸脱）をセーフエリア右端基準へ修正。実装判断 2 点を設計書へ反映: **型・難度ピルの表示文言はコード側定数 `PILL_LABELS`**（なぞなぞ・サクッと / フェルミ推定・じっくり / とんち・ひらめき。文言確認は 16-4）、**キャプションは 5 種以外の `{{...}}` トークンも異常終了**（タイポ検知）。ローカル ffmpeg（Docker）で実ストック + 実イラスト流用の 3 スロット分 16 秒 MP4 を生成し版面・尺・音声を確認（API 費ゼロ。`plans/16-3-trial/`）。**同日追記（動画のユーザーレビュー 3 点）**: ①バッジ修正は承認 ②**型・難度ピルは廃止**（`PILL_LABELS` ごと削除・「ない方がスッキリする」）③**`slot_label` を「朝 / 昼 / 夜の脳みそトレ」へ改名**（もはやロジトレではないため。ローカル MySQL 適用済み・**Aurora への反映は 16-4**）。デプロイ・テンプレート差し替え・実投稿・スケジュール再開は 16-4。詳細は [development-log.md](development-log.md) の 16-3 を参照
- [x] **16-3b** 動画事前生成方式への転換設計（設計 Fix）
  - 内容: 上記の仕様変更（2026-08-08 展開注記）を詳細設計へ落とし、関連設計書を改訂する。主論点: ①ストック ↔ 事前動画の紐づけ（`quiz_stock_items` へのカラム追加 = V006 か S3 命名規約のみか。在庫確認クエリの「動画未ビルト」検出を含む）②実行時の新方式（仮称 `quiz-prebuilt`）の契約実装 — ストック LRU 選定・S3 から事前動画取得（欠落はフェイルラウド）・`quiz_items` 記録とキャプション展開経路は不変・BGM ベイク済みに伴う `generated_media.audio_asset_id` の記録方法と選曲ローテーション廃止の扱い ③gpt-quiz-multicut の位置づけ変更（実行時方式としては休止・事前ビルドのレンダラーへ転用。方式カタログと方式設計書の改訂）④事前ビルドツーリングの設計 — imagegen 委譲（画風指定行の恒久化）→ 3:2 センタークロップ → 組版 + ffmpeg ビルド → レビューシート → S3 アップロード → DB 反映。置き場（`content/` 配下）と週次補充スキル `/quiz-stock-replenish` への組み込み ⑤S3 置き場はライフサイクル 30 日の対象外プレフィックス（`assets/` 系）とし、再利用時の同一動画再投稿の許容を decision 化 ⑥事業戦略書のコストモデル改訂（イラスト API 定常費のゼロ化）
  - 確認条件: 設計書（方式設計書・data-model.html・operation.html・セット別設計書・事業戦略書）が改訂され、16-3c の実装指示が書ける水準で Fix している
  - 備考: 2026-08-08 完了（Fable 5）。主な決定: **① V006 採用**（`video_s3_key` / `video_audio_asset_id` / `video_built_at` の 3 カラム。S3 命名規約のみ案は未ビルト検出が SQL で完結しないため不採用）**② 新方式設計書 [quiz-prebuilt.html](app/generators/quiz-prebuilt.html) を新規作成**（ビルド済み限定 LRU・S3 欠落フェイルラウド・`audio_asset_id` はストック行から転記〔ミュート検知クエリ無変更〕・中間生成物なし・共通骨格 / SNS 投稿バッチ / キャプション展開は無変更・切替は `generator_name` UPDATE 1 本で可逆）**③ gpt-quiz-multicut は実行時休止・レンダラー転用**（方式カタログ改訂・レジストリ残置）**④ ビルドツーリングは `content/video-build/<set_code>/`**（imagegen + 画風行恒久化 → 3:2 クロップ → レンダラー流用 → 全数レビュー → S3 + DB 反映。operation.html 手順 5 として正式化。版面 / スロット構成 / BGM 変更は再ビルドを伴う warn 化）**⑤ S3 は `assets/{set_code}/prebuilt/`**（ライフサイクル対象外・CDK 変更不要・同一動画再投稿の許容を decision 化・在庫確認クエリ拡張版）**⑥ コストモデル改訂**（イラスト API 0 円 + 16-1 の LLM 休止の反映漏れ回収 = logic-training-1 の生成 AI API 定常費は完全ゼロ）。設計書 7 本を改訂。詳細は [development-log.md](development-log.md) の 16-3b を参照
...
- [x] **16-4** 投稿前レビューと切替（15-14 パターン)
  ...
  - 備考: 2026-08-08 完了（Fable 5 が直接実施。Aurora 書き込み・RunTask・SFN 実行・cdk deploy はユーザー許可のうえ実行）。**切替日 = 2026-08-08（前後比較の基準点）**。デプロイ（16-3〜16-3d の 14 コミット push → 両パイプライン成功・初回は Docker Hub 429 でリトライ）→ Aurora 反映 4 点 → 手動 RunTask（run 59・noon・**exit 0・26 秒** = 生成 AI API 呼び出しゼロの軽量実行を実証。動画は 16-3d レビュー済み noon-001 と ETag 完全一致）→ 投稿前レビュー GO → SNS 投稿 SFN 単体実行 SUCCEEDED（リール + ストーリーズ両 success・キャプション 242 字プレースホルダ展開一致）→ スケジュール 3 本 ENABLED 化。**定時実行確認済み（08-08 21:00 night / 08-09 7:30 morning・12:30 noon の 3 スロットとも生成 + 投稿 SFN 成功・リール + ストーリーズ全 success・LRU 消費順どおり）**。詳細は [development-log.md](development-log.md) の 16-4 を参照
- [x] **16-4b** 切替後のフォーマット改善（視線誘導と見切れ解消）
  - 内容: 切替後の実物レビューで見つかった 2 点を直す。①**コーチ + 吹き出しのバウンド演出**（問題文を読む間ヒント・答えの所在に視線が向かない）②**イラストの見切れ解消**（レビュー画像では見えている被写体が動画で切れる）。既存 42 本の再ビルドと S3 差し替えを伴う
  - 確認条件: 42 本を再ビルドして人間レビュー（観点 = 動きの見え方・イラストの収まり）を通し、S3 と両環境 DB を差し替えても出題キュー（`last_used_at`）と選曲が変わらないこと
  - 備考: 2026-08-09 完了（Opus 5 が直接実施）。**①バウンド**: コーチ + 吹き出しを透過レイヤーへ分離し ffmpeg `overlay` の減衰サインでホップさせる方式を採用（試作 2 巡でユーザーがパラメータ確定: 振幅 150 / 34px・1.6 Hz・減衰 3.0・1.0 秒。ヒント登場と答え誘導を大・考えるへの切替を小・残りは静止）。大バウンド 2 箇所が既存 SE（8 秒 / 13 秒）と一致し音源追加ゼロで同期。**②見切れ**: 中央クロップが 2 段（intake の 3:2 化 + 組版の枠埋め）で元画像の幅の 12〜43% を失っており、問題文が長いほど被害大と全 42 問の計測で判明。組版側の枠埋めを廃止（`contain` で内接 + 枠内中央寄せ）し、intake の 3:2 は維持する案をユーザーがサンプル比較で選択。あわせて **imagegen の 3:2 出力をプロンプトで明示**（16-3c の「3:2 指定不可」は誤りと判明。同一セッション内で比率が揺れていただけで 42 問中 3 問は 1536x1024 で出力されていた）+ intake の切り捨て警告 + スキルへのサイズ確認手順を追加。**再ビルド**: `build.py --rebuild`（ビルド済み行対象・BGM は `video_audio_asset_id` 引き継ぎで LRU 非消費）を追加し 42 本を再ビルド → 全数レビュー承認 → S3 84 オブジェクト上書き + Aurora 42 行 + ローカル 42 行を反映。検証で未出題 39 問・BGM 3 種が差し替え前後で不変であることを確認（再投稿は発生しない）。詳細は [development-log.md](development-log.md) の 16-4b を参照
