...
- [ ] **4-6** Step Functions 手動実行による E2E 確認（WaitForDbReady → ECS タスク実行の一連フロー）
  ...
  - 備考: Phase 4-5 は Step Functions の追加デプロイのみ。4-6 は WaitForDbReady を含む一連フローの動作確認。手動実行 input は `{"set_code":"fashion-set-1"}` のように `set_code` を必ず渡す。Phase 5 で ImageBatchStack に EventBridge Scheduler を追加し、SNS 投稿は画像生成成功後に自動起動される
...
- [ ] **5-5** Step Functions ステートマシンを追加
  ...
  - 備考: ImageBatchStack の Step Functions は SnsPostBatchStack の Step Functions ARN を参照するため、Phase 4 完了が前提。手動実行 input は `{"set_code":"fashion-set-1","scheduled_at":"2026-04-19T00:00:00Z"}` のように `set_code` と `scheduled_at` を必ず渡す
