...
2026-07-24 のトークン最適化検討で策定。方針: **壁打ち・設計 Fix・大型委譲成果物のレビューは Fable 5、それ以外の定型ステップは `/model` で Opus 4.8 に切り替える**。Codex 委譲は terra / high を既定とし、機械的な横展開は luna、大型・新規性の高い実装は sol へ per-call で変更する（CLAUDE.md の Codex 連携ルールを参照）。エフォートは「確実にこなせる最低レベル」を選ぶ。
...
> 2026-07-26 にステップ展開（14-12 の事後確認〔初回定時実行〕のみ残した状態で先行実施。経緯・割り付けの詳細は [development-log.md](development-log.md) の Phase 15 冒頭を参照）。本フェーズは新規機能開発ではなく、**ドキュメント化済みのセット追加運用手順（[docs/app/operation.html](app/operation.html) セクション 2.1）の初回実地検証**が主軸（15-4 = 手順 0、15-2 = 手順 1、15-5 = 手順 2 + 手動の動作確認、15-6 = 手順 3、15-7 = 定時実行の確認）。2 セット並行運用に必要な構造は実装済み（投稿対象選定はセット単位、SFN・タスク定義・イメージは全セット共有）のため、コード変更は原則発生しない。15-2 以降は標準手順に基づく暫定展開で、15-1 の壁打ち結果を受けて確定する（新生成方式・AI 音楽生成の採用が決まった場合はステップを追加）。外部作業を含む 15-2・15-3 は 15-1 完了後であれば後続と並行して進めてよい（14-6 と同じ扱い）。

- [ ] **15-1** テーマ選定の壁打ちと実装ステップの確定
  - 確認: 「リーチを狙える × 独自の世界観」（[事業戦略書](strategy/business-strategy.html) セクション 2 のオリジナリティ対策要件）を満たすテーマ・`set_code`・生成方式（gpt-image-kenburns 流用 or 新方式）・音源方針（AI 音楽生成〔Suno 等・単発費用〕への格上げ要否。同セクション 3.1 の段階方式）・投稿頻度・アカウント構成が決定され、壁打ち記録 1 本 + 事業戦略書セクション 6（セットポートフォリオ）が更新されている。決定を受けて 15-2 以降のステップとモデル・エフォート割当が確定している

- [ ] **15-2** Instagram アカウントの開設と Secret 登録（外部作業含む）
...
- [ ] **15-3** 音源の調達と登録（外部作業含む）
  - 確認: 15-1 の音源方針に沿った 3〜5 曲が前処理（14-6 で確立した 2 パス loudnorm レシピ）済みで S3 `audio/<set_code>/` に配置され、ライセンス証跡込みで `audio_assets` にローカル & Aurora とも登録されている

- [ ] **15-4** セット別設計書の作成とプロンプト・キャプション設計
  - 確認: `docs/app/sets/<set_code>.html` が作成され（operation.html 2.1 手順 0 の雛形）、実 Images API の試し打ちでテーマの構成・世界観が安定して再現される（14-11 のプロンプト設計の知見〔構成の明示・Instagram UI セーフエリア〕を流用）。キャプションテンプレート案に `#AIart` が含まれている

- [ ] **15-5** セット登録と投稿前レビュー（試し生成・実投稿）
  - 確認: DB 登録（`batch_sets` → `prompt_configs` → `caption_templates` → `sns_accounts` → `audio_assets` の順。ローカル & Aurora）後、image-batch の直接 RunTask で生成物を目視レビューし、SNS 投稿 SFN の単体実行で新アカウントへのリール実投稿が success になる（14-11 で実績のある投稿前レビュー方式）

- [ ] **15-6** Scheduler 追加と命名規約の確定
  - 確認: セット別 Scheduler の命名規約が確定し（設計課題リスト 2026-07-15。既存 `acps-prod-image-generation-schedule` のリネーム要否の判断を含む）、勝負セットの Scheduler が CDK で追加・deploy・ENABLED 化されている。workflow.html・operation.html の記述が実態と一致している

- [ ] **15-7** 定常運用切替と締め（2 セット並行運用・設計書整備）
  - 確認: 2 セット並行の定時実行が双方とも成功して自動運用に入り（KPI M2 への挑戦開始）、セット別設計書の Fix・事業戦略書の更新・トークン失効監視の自動化の要否判断（設計課題リスト 2026-07-07）・設計課題リストの棚卸しが済んでいる
...
| 2026-07-25 | database/V001__initial_schema.sql（`generated_media.file_format`） | カラム COMMENT が「ファイル形式（Instagram 要件により jpg に変換して保存する）」のままで、動画（`mp4`）を含む 14-4 以降の実態とずれている。型は `VARCHAR(20)` のため動作影響はない | 次に DDL を追加する機会（V003 以降）で COMMENT を更新する（14-12 の棚卸しで継続を確認。単独で V003 を起こすほどではない） | V003 追加時 |
