...
- [ ] **14-4** 動画対応の設計 Fix（データモデル・方式契約・投稿フロー・運用ルール）
  ...
  - 備考: 決める内容 — 動画メタ情報の置き場（壁打ちの候補 a/b/c）と `post_images` の扱い、音源テーブル（仮称 `audio_assets`）の定義・選曲アルゴリズム・ライセンス証跡の置き場、S3 キー規約の拡張（動画・音源ストック）、`posts` のメディア種別の表現、方式契約の一般化（「最終メディアを返す」）の要否と方式名（レジストリキー）、ffmpeg 合成パラメータ（出力 1080x1920・fps・ビットレート =「標準画質」の定義、ズーム/パン演出）、音源前処理（切り出し・音量正規化）の役割分担（事前手動 or バッチ内）、AI 開示ハッシュタグの選定、リールのカバー画像の方針。動画方式の方式設計書（`docs/app/generators/`）は「本採用時に作成」ルールに従いここでは作らず 14-11 で新設する
...
- [ ] **14-6** 音源の調達と登録（外部作業含む）
  ...
  - 備考: **業務データ投入ポイント③**: 実際に使う<strong>音源ファイル</strong>をここで投入する（選曲はユーザー決定、Claude が候補サイト・候補曲の提示可。CC-BY はクレジット運用が要るため CC0 優先）。前処理の内容・証跡の記録先は 14-4 の決定に従う。選曲（外部作業）は 14-4 完了後に先行着手してよい（登録は 14-5 のテーブル作成後）
...
- [ ] **14-7** 動画方式 strategy の実装（Codex 委譲）
  ...
  - 備考: `generators/` に動画方式モジュールを追加しレジストリ登録（gpt-image-single は残置。壁打ち決定事項 #5）。image-batch の Dockerfile に ffmpeg を追加。共通骨格・方式契約の変更は 14-4 の決定に従う。Fargate タスクの CPU / メモリ / 一時ストレージの見直し（CDK・ImageBatchStack）が必要ならここで行う。ローカルの `prompt_configs` の size 変更（1024x1536）もここで実施（Aurora 側は 14-10 の切替時）
...
- [ ] **14-10** AWS E2E（セット切替 + 全チェーンでリール実投稿）
  ...
  - 備考: push で両パイプラインをデプロイ → Aurora のレコード切替（`batch_sets.generator_name` 差し替え・`prompt_configs` の size 変更・`caption_templates` への AI 開示ハッシュタグ反映）→ SFN 手動実行。切替後は定時実行（1 日 3 回のまま）も動画で走るため、14-11 までの間隔・一時的な扱い（そのまま走らせる / 一時 DISABLE）をここで判断する
...
- [ ] **14-11** 定常運用切替と締め（Scheduler 1 日 1 回化・設計書整備）
  ...
  - 備考: Scheduler の cron 変更（3 回 → 1 回）+ ImageBatchStack デプロイ。13-2 と同じ締めの位置づけ: 動画方式の方式設計書を `docs/app/generators/` に新設（「本採用時に作成」ルール。実装を正として記述）、方式カタログのステータス更新、[sets/fantasy-animals-1.html](app/sets/fantasy-animals-1.html) の更新（方式・頻度・音源・キャプション）、事業戦略書の KPI 実績表更新（M1）、設計課題リストの棚卸し。Phase 15 のステップ展開は本ステップ完了後に行う
...
- 骨子: テーマ選定（「リーチを狙える × 独自の世界観」を要件とする。事業戦略書セクション 2 のオリジナリティ対策）→ セット追加運用の実地検証 → 投入。Scheduler 命名規約の確定（設計課題リスト 2026-07-15）とトークン失効監視の自動化の要否判断（同 2026-07-07）を含める。ステップ展開は Phase 14 完了時に行う
