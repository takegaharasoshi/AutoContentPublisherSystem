...
完了ステップ（Phase 14〜16-4b）の割当実績は [development-log.md](development-log.md) の各ステップ記録と git 履歴を参照。残ステップの割当:

| ステップ | Claude 側（モデル / エフォート） | Codex 委譲（モデル / エフォート） |
|---|---|---|
| 16-5 効果検証 | Opus 5 / high（前後比較の解釈で詰まったら Fable 5 へ切替） | なし（インサイト画面の目視のため委譲不向き） |
| 16-6 Graph API インサイト調査 | **Fable 5** / デフォルト（委譲結果の再検証・設計への含意整理） | Web リサーチを `codex --search exec` で委譲（terra / high） |
| 16-7 インサイト収集バッチの壁打ち・設計 Fix | **Fable 5** / デフォルト | なし（実装ステップは 16-8 以降の展開時に割当） |
...
- 骨子: 第 3 のバッチ（インサイト収集）の設計・実装（A-1 壁打ちから将来拡張として織り込み済み。データモデルは `posts.id` を FK にした新規テーブル追加で対応する想定〔[data-model.html](app/data-model.html) セクション 3〕・問題別パフォーマンスの突合キーは `posts.stock_item_id`〔16-1 整備済み〕）。2026-08-09 のユーザー決定で 16-5 完了を待たず先行着手（並行方針の理由は Phase 16 冒頭の備考を参照）
- [ ] **16-6** Instagram Graph API インサイト機能の調査
  - 内容: リール・ストーリーズのメディアレベルで取得可能なメトリクス（視聴数・リーチ・視聴時間・視聴維持やスキップ率に相当する指標の有無）、アカウントレベルのメトリクス（フォロワー数・増減等）、必要パーミッションとアプリレビュー要件、レート制限、データ保持期間（ストーリーズの取得期限・メディアの遡及可能範囲）、2025〜2026 年のメトリクス廃止・改名動向を調査する（Codex Web リサーチ委譲 + Fable 5 再検証）。成果物は「自動収集できる指標 / 手動でしか見えない指標」の境界の確定。16-5 の手動比較で記録すべき項目への入力を兼ねる
- [ ] **16-7** インサイト収集バッチの壁打ち・設計 Fix
  - 内容: 第 3 バッチの実行基盤（スタック構成・スケジュール・ワークフロー。既存 2 バッチのパターン踏襲が既定）・データモデル拡張・収集方式を壁打ちして設計 Fix する。収集は「API で取得可能な生メトリクスの全量収集」を既定案とし、指標選定への依存を収集側から切り離す。**「継続的に見る指標セット」（可視化・KPI 転記の対象）の確定のみ 16-5 完了後に反映する**（2 段階 Fix）
- [ ] **16-8 以降** 実装・稼働確認（16-7 の設計 Fix 時にステップ展開。Codex 委譲割当も同時に行う）
