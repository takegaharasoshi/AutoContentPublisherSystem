...
- **インフラから先に構築し、アプリ（業務ロジック）の設計・実装は後から行う**
  - Phase D: インフラ設計の一時 Fix → Phase 0〜8: インフラ構築（空回し確認・監視・CI/CD まで）→ Phase 9: アプリ設計 → Phase 10 以降: アプリ実装
...
**ゴール**: 生成 AI との対話でアプリ設計書（docs/app/）を作成し、一時 Fix する

> ここまでのフェーズでインフラは完成し、空回しでの動作イメージが得られている。その理解をもとにアプリ仕様を固める。[docs/_archive/](_archive/) の旧設計（batch.md / database.md / operation.md）を参考資料として活用する。

- [ ] **9-1** 仕様イメージの壁打ち
  - 確認: [docs/app/index.html](app/index.html) の検討メモにユースケース・要件が言語化されている
  - 備考: 生成 AI と対話しながら、投稿運用のイメージ・セットの考え方・失敗時にどうしたいか等を言語化する。**設計書はまだ書かない**

- [ ] **9-2** バッチ処理フロー設計
  - 確認: docs/app/ に処理フロー設計書（HTML）が作成されている
  - 備考: 冪等性・再試行・投稿ステータス管理・二重投稿防止・バッチサイズ制限を扱う。docs/_archive/batch.md を参考資料とする

- [ ] **9-3** DB スキーマ設計 + 本スキーマ DDL 作成
  - 確認: docs/app/ に DB 設計書（HTML）が、`database/` に本スキーマの DDL がある
  - 備考: docs/_archive/database.md を参考資料とする。DDL のバージョン管理・マイグレーション方針もここで定義する

- [ ] **9-4** アプリ運用・セキュリティ（アプリ部分）の設計
  - 確認: docs/app/ にアプリ運用設計書（HTML）が作成されている
  - 備考: プロンプト管理、SNS アカウント追加手順、投稿失敗時の手動補正、SNS Secret 規約の最終確認（[docs/infra/security.html](infra/security.html) セクション 1.2 の注記）。docs/_archive/operation.md を参考資料とする
