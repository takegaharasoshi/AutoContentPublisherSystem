...
- [x] **16-11** 再認可・稼働確認・バックフィル
  - 備考: 2026-08-10 完了（Opus 5。ブラウザ操作はユーザー・検証と修正・デプロイは Claude）。`instagram_manage_insights` 込みで再認可し 2 件の Secret を更新（`data_access_expires_at` 2026-10-25 → **2026-11-08**・リマインダーは 2026-11-01 へ）。両セットの手動実行が成功し、**リール 33 件 + アカウント日次 90 日 × 2 アカウントを収集**（`reels_skip_rate` の実データ取得を確認）。疎通確認で**閲覧者不足によるインサイト非開示（`(#10) Not enough viewers`）が失敗計上され恒常アラーム化する不具合**を発見し、スキップ扱いへ修正のうえ再デプロイ。バックフィルの結果、**8/8 切替前の logic-training-1 リール 16 件は Instagram 上に存在せず自動収集できない**ことが判明（16-5 の前後比較は手動記録が正）。Scheduler 4 件は ENABLED へ復帰し**インサイト収集が定常運用入り**。詳細は [development-log.md](development-log.md) の 16-11 を参照
