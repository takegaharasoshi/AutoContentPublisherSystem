...
| 2026-07-25 | database/V001__initial_schema.sql（`generated_media.file_format`） | カラム COMMENT が「ファイル形式（Instagram 要件により jpg に変換して保存する）」のままで、動画（`mp4`）を含む 14-4 以降の実態とずれている。型は `VARCHAR(20)` のため動作影響はない。次に DDL を追加する機会（V003 以降）で COMMENT を更新する |
