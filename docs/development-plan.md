...
- [x] **17-1** テーマ Fix 壁打ち（15-2 相当）
  ...
  - 備考: 2026-08-10 完了（壁打ち 3 巡 + キャラ試し生成 3 巡〔Codex imagegen〕+ パレット版面モック比較〔Remotion 実レンダリング〕+ VOICEVOX 4 候補試聴）。決定: フォーマット軸 = TOP5 地図ルーレット型（全県塗り型は 17-2 で設計のみ・行型は部品へ吸収）/ 尺 = **20 秒・30 秒の 2 尺・両方フルナレーション（方針⑧を修正）**/ キャラ =「**表彰台五郎**」（表彰台擬人化 + 赤鉢巻 + マイク・1 位発表時のみ軍配。ナレーター専任 + 画面隅常駐）/ 声 = **VOICEVOX 白上虎太郎** / パレット = **和モダン系**（生成り × 藍 × 朱 × 金）/ 背景 = 生成り地 × 和風モチーフ四隅 × 中央無地 / `set_code` = **`pref-ranking-1`** / アカウント =「表彰台五郎の都道府県ランキング」（`daigoro_ranking` 第一候補）。壁打ち記録 [requirements-notes-set3.html](app/requirements-notes-set3.html) を新設し、事業戦略書（ポートフォリオ・コスト・KPI 実績）へ追記済み。詳細は [development-log.md](development-log.md) の 17-1 を参照
