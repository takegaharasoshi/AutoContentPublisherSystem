...
> **再試行の仕組み**: 再試行は新規レコード（`attempt_number` インクリメント）で管理する。`failed` レコードは履歴として保持され、次回バッチ実行時に `status='success'` または `status='published_unconfirmed'` が存在しなければ新しい `pending` レコードが作成される。`attempt_number` が `batch_sets.max_post_retries`（デフォルト: 3）以上の場合はスキップされ、ログに警告が出力される。既存の `pending` レコードの中断復旧は retry 上限チェックより優先する。
...
> **一意性制約の理由**: Step Functions Retry により同じ `EXECUTION_ARN` のコンテナが再起動された場合でも、`UNIQUE (execution_arn, batch_type)` により実行ログの二重作成を防ぐ。制約に衝突した場合は既存レコードを取得して UPDATE する。詳細は [design/batch.md](../design/batch.md) セクション 1.9 を参照。
...
`prompt_configs.parameters` は画像生成 API に渡す生成設定を JSON で保持する。初期実装では以下のキーを扱う。
