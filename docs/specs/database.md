...
| updated_at | DATETIME | NO | CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP | 更新日時（UTC） |
...
| parameters | JSON | YES | NULL | 生成パラメータ（モデル、アスペクト比、画像サイズ等）。スキーマはセクション 7 を参照 |
| is_active | TINYINT(1) | NO | 1 | 有効フラグ |
| created_at | DATETIME | NO | CURRENT_TIMESTAMP | 作成日時（UTC） |
| updated_at | DATETIME | NO | CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP | 更新日時（UTC） |
...
- **UNIQUE**: `uq_prompt_configs_set_id` (`set_id`, `id`) — 複合 FK の参照先として使用。左端プレフィックスにより `set_id` 単体の検索にも利用される
...
| generation_api_response | JSON | YES | NULL | API レスポンス（参考用）。保存前に認証情報・署名付き URL・個人情報に該当し得る値を除外する |
...
- **UNIQUE**: `uq_generated_images_set_id` (`set_id`, `id`) — 複合 FK の参照先として使用。左端プレフィックスにより `set_id` 単体の検索にも利用される
...
- **PK**: `id`
- **FK**: `set_id` → `batch_sets.id`
- **UNIQUE**: `uq_sns_accounts_set_id` (`set_id`, `id`) — 複合 FK の参照先として使用。左端プレフィックスにより `set_id` 単体の検索にも利用される
...
SNS 投稿の実行結果を管理する。再試行のたびにレコードを追加し、`status='success'` または `status='published_unconfirmed'` のレコードの有無で自動投稿の終端状態を判断する。
...
| status | ENUM('pending', 'success', 'failed', 'published_unconfirmed') | NO | | 投稿結果。`published_unconfirmed` は投稿済みの可能性があり自動再試行しない終端状態 |
...
| api_response | JSON | YES | NULL | API レスポンス（参考用）。保存前に認証情報・署名付き URL・個人情報に該当し得る値を除外する |
...
- **UNIQUE**: `uq_post_records_image_account_attempt` (`generated_image_id`, `sns_account_id`, `attempt_number`) — 同一 attempt の二重 INSERT を DB レベルで防止。左端プレフィックスにより `(generated_image_id, sns_account_id)` の検索にも利用される
...
> **再試行の仕組み**: 再試行は新規レコード（`attempt_number` インクリメント）で管理する。`failed` レコードは履歴として保持され、次回バッチ実行時に `status='success'` または `status='published_unconfirmed'` が存在しなければ新しい `pending` レコードが作成される。`attempt_number` が `batch_sets.max_post_retries`（デフォルト: 3）以上の場合はスキップされ、WARNING ログと `PostRetryExceededCount` カスタムメトリクスを出力する。既存の `pending` レコードの中断復旧は retry 上限チェックより優先する。

> **published_unconfirmed の扱い**: `published_unconfirmed` は、publish API 要求後のタイムアウトや重複 publish 応答などにより、投稿済みの可能性があるが `platform_post_id` を確定できない状態を表す。二重投稿回避のため、当該 `(generated_image_id, sns_account_id)` について自動再試行・新規 attempt 作成を行わない。運用者が Instagram 側を確認し、投稿済みなら `platform_post_id`、`posted_at`、`status='success'` を補正する。未投稿で再実行する場合は原因調査後に手動で状態を補正する。
...
> 3. `status='success'` または `status='published_unconfirmed'` が存在すればスキップ、存在しなければ `pending` レコードを INSERT する
...
| attempt_count | INT UNSIGNED | NO | 1 | 同一 Step Functions 実行内で ECS タスクが起動された回数 |
...
- **UNIQUE**: `uq_batch_execution_logs_execution_type` (`execution_arn`, `batch_type`) — Step Functions 経由の同一実行・同一バッチ種別の二重 INSERT を防止する。MySQL の UNIQUE は NULL を複数許容するため、手動 RunTask（`execution_arn` が NULL）では複数レコードを登録できる

> **更新責務**: 各バッチアプリケーションが開始時に `status='running'`, `attempt_count=1` を INSERT し、終了時に `succeeded` または `failed` へ UPDATE する。`execution_arn` には Step Functions から渡される `EXECUTION_ARN` を格納し、手動 RunTask 時は `NULL` を許容する。

> **Retry 時の更新**: Step Functions Retry により同じ `EXECUTION_ARN` のコンテナが再起動され、`UNIQUE (execution_arn, batch_type)` に衝突した場合は、既存レコードを `status='running'`, `finished_at=NULL`, `attempt_count=attempt_count+1` に更新して再利用する。既存レコードが `succeeded` の場合は再処理せず正常終了する。詳細は [design/batch.md](../design/batch.md) セクション 1.9 を参照。

> **records_processed の定義**: 画像生成バッチでは最終的に生成完了した画像件数、SNS 投稿バッチでは最終 attempt で処理した投稿対象件数を格納する。
...
- `CURRENT_TIMESTAMP` は DB セッションのタイムゾーンに依存するため、アプリケーションは DB 接続直後に `SET time_zone = '+00:00'` を実行する
- Aurora 側でも可能な限り Parameter Group で `time_zone='UTC'` を設定し、手動 DDL / Query Editor 実行時も UTC 前提を維持する
...
### 7.1 prompt_configs.parameters

`prompt_configs.parameters` は画像生成 API に渡す生成設定と、S3 保存時の内部設定を JSON で保持する。初期実装では以下のキーを扱う。

| キー | 型 | 必須 | デフォルト | 説明 |
|---|---|---|---|---|
| model | string | NO | `gemini-3-pro-image-preview` | 画像生成モデル名 |
| aspect_ratio | string | NO | `1:1` | 画像のアスペクト比。例: `1:1`, `4:5`, `16:9` |
| image_size | string | NO | `1K` | 出力画像サイズ。例: `1K`, `2K`, `4K` |
| output_format | string | NO | `jpg` | S3 保存時の形式。Instagram 投稿要件に合わせ初期値は `jpg` |

`model`、`aspect_ratio`、`image_size` は画像生成 API に渡す。`output_format` は API には渡さず、API から返った画像を S3 保存前に変換するための内部設定として扱う。
...
未定義キーは初期実装では無視する。将来のパラメータ追加時は、本セクションを更新してからアプリケーションで利用する。
