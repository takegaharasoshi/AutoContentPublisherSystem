...
| S3 Bucket | 画像保存用（Lifecycle Policy: 30 日で自動削除）。Block Public Access 有効、S3 管理キーによるサーバーサイド暗号化、SSL 強制を設定する |
| Aurora Serverless v2 | MySQL 互換 DB（自動一時停止有効、最小 ACU は 0、最大 ACU は 1.0。Aurora MySQL 3.08.0 以降など対応バージョンを採用）。CDK 引数指定: `serverlessV2MinCapacity: 0`、`serverlessV2MaxCapacity: 1.0`、`defaultDatabaseName: "acps"`、`enableDataApi: true`（Query Editor / Data API 利用のため必須）、`deletionProtection: true`、`removalPolicy: RETAIN`。自動バックアップ保持期間は 7 日に設定する。Parameter Group で `time_zone='UTC'` を設定する |
...
> **注記**: 以下は ImageBatchStack の最終形である。Phase 5 の空回し段階では ECS Task Definition / Step Functions / EventBridge Scheduler を先に作成し、CodePipeline / CodeBuild は Phase 10 で追加する。
...
> **注記**: 以下は SnsPostBatchStack の最終形である。Phase 4 の空回し段階では ECS Task Definition / Step Functions を先に作成し、CodePipeline / CodeBuild は Phase 10 で追加する。
