...
画像生成を行う ECS Fargate RunTask 用バッチです。Phase 6 では Aurora（MySQL）への接続確認として、`connection_test` テーブルへの INSERT / SELECT を実行します。
...
```bash
cd services/image-batch && uv run pytest
```
...
```bash
docker build -f services/image-batch/Dockerfile -t image-batch .

export ENV_NAME=local
export DB_SECRET_JSON='{"username":"app","password":"password","host":"host.docker.internal","port":3306,"dbname":"acps"}'
docker run --rm -e ENV_NAME -e DB_SECRET_JSON image-batch
```
...
docker build --platform linux/amd64 --provenance=false -f services/image-batch/Dockerfile -t "${IMAGE}:${TAG}" .
