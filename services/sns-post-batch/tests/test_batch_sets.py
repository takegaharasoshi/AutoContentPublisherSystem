...
def test_find_batch_set_returns_none_when_not_found() -> None:
    ...
    cursor.execute.assert_called_once_with(
        "SELECT id, set_code, is_active, stories_enabled, "
        "problem_snapshot_enabled FROM batch_sets "
        "WHERE set_code = %s",
        ("missing",),
    )
...
def test_find_batch_set_normalizes_active_flag() -> None:
    ...
    cursor = Mock()
    cursor.fetchone.return_value = (3, "set-a", 0, 1, 1)

    assert find_batch_set_by_code(cursor, "set-a") == BatchSet(
        3, "set-a", False, True, True
    )
